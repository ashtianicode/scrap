...
for url in urls :
               ...
               try:
                    ...
                    page=requests.get("http://www.asnafyab.ir/" + url)
                    soup = bs4.BeautifulSoup(page.content,"lxml")
                    ...
                    for a in soup.findAll("div", { "class" : "JobDetail" }) : 
                               ...
                               for s in spans:
                                   ...
                                   if s == subjects[1]:
                                       index = spans.index(s)
                                       sharh= spans[index+1].text
                                        
                                   if s == subjects[3]:
                                       index = spans.index(s)
                                       modir= spans[index+1].text
    
    
                                   if s == subjects[5]:
                                       index = spans.index(s)
                                       tamas = spans[index+1].text
    
                                   if s == subjects[7]:
                                       index = spans.index(s)
                                       hamrah= spans[index+1].text
    
                                   if s == subjects[9]:
                                       index = spans.index(s)
                                       mantaghe= spans[index+1].text
